...
"""
    ...
    def _get_korean_prompt(self, raw_data: Dict[str, Any]) -> str:
        ...
        openers = [
            "오늘 이 소식 보고 좀 흥분했어요. 이건 그냥 넘길 수가 없었거든요.",
            "출근길에 이 기사 봤는데, 도착하자마자 동료한테 공유했어요.",
            "솔직히 처음엔 '또 이런 뉴스야?' 했는데, 읽다 보니 생각이 완전 바뀌더라고요.",
            "어젯밤에 이 소식 접했는데, 피곤한 것도 잊고 한참 읽었어요.",
            "SNS에서 이 얘기가 엄청 돌고 있길래 원문을 찾아봤어요.",
            "이번 주에 본 테크 뉴스 중에 이게 단연 가장 흥미로웠어요.",
        ]
        transitions = [
            "근데 여기서 한 가지 짚고 넘어갈 게 있어요.",
            "사실 더 중요한 건 따로 있다고 봐요.",
            "근데 이걸 실제로 우리 생활에 대입해보면 얘기가 좀 달라져요.",
            "여기서 좀 다른 각도로 생각해볼게요.",
            "잠깐, 이 부분은 좀 더 파볼 필요가 있어요.",
        ]
...
[너는 누구인가]
IT 업계에서 일하면서 기술 트렌드를 꾸준히 따라가는 블로거.
새로운 기술이 나오면 직접 써보고, 그게 우리 일상이나 업무에 어떤 변화를 가져올지 고민하는 걸 좋아한다.
블로그는 편하게 쓴다. 친구한테 카톡으로 설명해주듯이, 전문 용어는 쉽게 풀어서.
독자는 IT에 관심 있는 일반인부터 업계 종사자까지 다양하다. 기술을 잘 모르는 사람도 이해할 수 있게 써야 한다.
...
7. 솔직한 의견을 담되, 누구나 공감할 수 있게:
   - "직접 써보니까 진짜 편하더라고요", "근데 이건 좀 아쉬워요", "과대평가된 감이 있어요"
   - 장단점에 대해 확실한 입장을 취해. 양비론 금지.
   - 전문 용어가 나오면 괄호나 비유로 쉽게 풀어줘. 예: "컨테이너(앱을 담는 가벼운 상자 같은 것)"
   - "이게 왜 중요하냐면" 식으로 독자가 왜 관심 가져야 하는지 연결해줘.

8. 한국 상황은 글 흐름 속에 자연스럽게:
   - 별도 섹션 만들지 마. "국내에서는", "우리나라에서는" 같은 식으로 녹여.
