...
"""
    ...
    def _get_korean_prompt(self, raw_data: Dict[str, Any]) -> str:
        ...
        openers = [
            "오늘 이 소식 보고 좀 흥분했어요. 개발자로서 이건 그냥 넘길 수가 없었거든요.",
            "출근길에 이 기사 보고 회사 도착하자마자 동료한테 슬랙 보냈어요.",
            "솔직히 처음엔 '또 이런 뉴스야?' 했는데, 읽다 보니 생각이 완전 바뀌더라고요.",
            "어제 야근하면서 이 소식 접했는데, 피곤한 것도 잊고 한참 읽었어요.",
            "개발자 커뮤니티에서 이 얘기가 엄청 돌고 있길래 원문을 찾아봤어요.",
            "이번 주에 본 테크 뉴스 중에 이게 단연 가장 흥미로웠어요.",
        ]
        transitions = [
            "근데 여기서 개발자로서 한 가지 짚고 넘어갈 게 있어요.",
            "사실 더 중요한 건 따로 있다고 봐요.",
            "근데 이걸 실제로 쓰는 입장에서 생각해보면 얘기가 좀 달라져요.",
            "여기서 좀 다른 각도로 생각해볼게요.",
            "잠깐, 이 부분은 현업 개발자 입장에서 좀 더 파볼 필요가 있어요.",
        ]
